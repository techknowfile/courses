...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        for index, param_var in enumerate(param_vars):
            updates.append((param_var, param_var - self.learning_rate*grad_tensors[index]))
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        for index,param_var in enumerate(param_vars):
            vt1 = (self.momentum*vel_vars[index]) - (self.learning_rate*grad_tensors[index])
            updates.append((vel_vars[index], vt1))
            updates.append((param_var, param_var + vt1))
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        _train =  {
            model.input_ph: input_train_data,
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss = session.run(loss_tensor, _train)
            train_losses.append(train_loss)
            session.run(update_ops, _train)
            val_loss = session.run(loss_tensor, _value)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        val_data = [input_val_data, target_val_data]
        # You may want to initialize some variables that are shared across iterations
        value_generator = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        train_generator = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            _train = {
                model.input_ph: train_next[0],
                target_ph: train_next[1]
            }
            train_loss = session.run(loss_tensor, _train)
            train_losses.append(train_loss)
            session.run(update_ops, _train)
            value_next = value_generator.next()
            _value = {
                model.input_ph: value_next[0],
                target_ph: value_next[1]
            }
            val_loss = session.run(loss_tensor, _value)
            val_losses.append(val_loss)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        val_data = [input_val_data, target_val_data]
        # You may want to initialize some variables that are shared across iterations
        value_generator = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        train_generator = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        loss_tensor = self.get_loss_tensor(model.prediction_tensor, target_ph, model.get_param_vars(regularizable=True))
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]
        train_losses = []

...
def moveFilesToTestFolder(student_folder, test_files):
    ...
    if not os.path.exists("tmp/"+'mdp.py'):
        shutil.copy(os.path.join('reinforcement', 'mdp.py'), 'tmp/' + 'mdp.py')
    if not os.path.exists("tmp/learningAgents.py"):
        shutil.copy(os.path.join('reinforcement', 'learningAgents.py'), 'tmp/' + 'learningAgents.py')
    if not os.path.exists("tmp/util.py"):
        shutil.copy(os.path.join('reinforcement', 'util.py'), 'tmp/' + 'util.py')
...
def main(args):
    ...
    with open('results.csv', 'w+') as f:
        ...
        for student_folder in student_folders:
            ...
            dt_due = datetime.datetime(2017, 10, 31, 23, 59, 59)
            dt_diff = dt_due - dt_submission
            if dt_diff.days < 0:
                late_days = -1*dt_diff.days
            else:
                late_days = 0
            moveFilesToTestFolder(student_folder, ["valueIterationAgents.py", "qlearningAgents.py", "analysis.py", "util.py"])
            ...
            total_grade = None
            if grades:
                question_grades = re.findall(r"Question q\d+: (.*)/(\d+)", grades)
                total_tuple = float( re.search("Total: (\d+)/(\d+)", grades).group(1) )
                grade = sum([float(i) for i,j in question_grades])

...
def findIsolatedZeros(features, visited, rows, columns):
    for i in range(rows):
...
def outOfBound(row, column, rows, columns):
	return row <0 or column <0 or row >= rows or column >= columns;

def visitCell(row, column, features, visited, rows, columns):
    if outOfBound(row, column, rows, columns):
        return
    if features[row][column] == 1 or (features[row][column] == 0 and visited[row][column] == True):
        return

    visited[row][column] = True
    for val in ARR_ITER:
		visitCell(row+val[0], column+val[1], features, visited, rows, columns)
    return
...
def enhancedFeatureExtractor(datum):
    ...
    _x, _y = findIsolatedZeros(features, visited, rows, columns)

    while _x != None:
        zeros += 1
        visitCell(_x, _y, features, visited, rows, columns)
        _x, _y = findIsolatedZeros(features, visited, rows, columns)
    ...
    features = np.concatenate((features.flatten(), _feature), axis = 0)
